...
public class ClassTest extends ClassTestBase {
  ...
  @Test
  public void testInterfaceExtendingAutoCloseable() throws Exception {
    ClassModel model = new Generator().generateClass(InterfaceExtendingAutoCloseable.class);
    ApiTypeInfo apiType = (ApiTypeInfo) model.getType();
    assertTrue(apiType.isAutoCloseable());
    assertFalse(apiType.isWriteStream());
    assertNull(apiType.getWriteStreamArg());
    assertFalse(apiType.isHandler());
    ClassModel model2 = new Generator().generateClass(InterfaceWithDefaultMethod.class);
    ApiTypeInfo apiType2 = (ApiTypeInfo) model2.getType();
    assertFalse(apiType2.isAutoCloseable());
    assertFalse(apiType2.isWriteStream());
    assertNull(apiType2.getWriteStreamArg());
    assertFalse(apiType2.isHandler());
  }
