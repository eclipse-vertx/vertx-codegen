 ...
 */
public class DefaultMethodCreator implements MethodCreator {

  private static final Logger logger = Logger.getLogger(DefaultMethodCreator.class.getName());
  ...
  public DefaultMethodCreator(ProcessingEnvironment env, boolean concrete) {
  ...
  @Override
  public Optional<MethodInfo> createMethod(
    TypeElement modelElt,
    ExecutableElement modelMethod,
    boolean allowAnyJavaType,
    Set<ClassTypeInfo> collectedTypes,
    Text deprecatedDesc
  ) {
    Doc.Factory docFactory = new Doc.Factory(env.getMessager(), elementUtils, typeUtils, typeFactory, modelElt);
    ...
    //
    List<ParamInfo> mParams = getParams(modelElt, modelMethods, modelMethod, paramDescs, allowAnyJavaType);
  ...
  private List<ParamInfo> getParams(TypeElement elem,
                                    List<ExecutableElement> modelMethods,
                                    ExecutableElement methodElt,
                                    Map<String, String> descs,
                                    boolean allowAnyJavaType) {
    ExecutableType methodType = (ExecutableType) typeUtils.asMemberOf((DeclaredType) elem.asType(), methodElt);
    ...
    for (int i = 0; i < params.size(); i++) {
      ...
      Text text = desc != null ? new Text(desc).map(Token.tagMapper(elementUtils, typeUtils, elem)) : null;
