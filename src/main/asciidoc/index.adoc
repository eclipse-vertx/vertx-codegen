...
When a data object has an associated `JsonDeserializer` (or inherited via `JsonMapper`), it can be instantiated from a json value
and is said _deserializable_. For POJO to json object mapping, although there are no strict rules of mapping between the data
...
When a data object has an associated `JsonSerializer` (or inherited via `JsonMapper`), it can be converted to the json format
and is said _serializable_. Vert.x API types have restriction in the declared method return types, a jsonifiable
